...
function createButton(buttonName) {
  ...
  return button;
} // Buton ekleme fonskiyonunda istediğimiz ismi verdiğimizde buton içinde yazacak. 
...
addButton(); // Butonları çağırma kısmı.

function foodAdd(name) {
...
// Tüm menü fonksiyonu
function addAll(){
  sectionDOM.innerHTML = ""; // Bu kod satırını eklemezsek gelen yemekler üst üste birikecek. Biz burada section kısmını sıfırladık.
  menu.forEach(items => {
    sectionDOM.innerHTML += (" " + foodAdd(items))
  });
}

// Korea menüsü fonksiyonu
function addKorea(){
  sectionDOM.innerHTML = "";
  const korean = menu.filter(meal => meal.category == "Korea");
  korean.forEach(items => {
    sectionDOM.innerHTML += (" " + foodAdd(items))
  });
}

// Japan menüsü fonksiyonu
function addJapan(){
  sectionDOM.innerHTML = "";
  const japanese = menu.filter(meal => meal.category == "Japan");
  japanese.forEach(items => {
    sectionDOM.innerHTML += (" " + foodAdd(items))
  });
}

// China menüsü fonksiyonu
function addChina(){
  sectionDOM.innerHTML = "";
  const chinese = menu.filter(meal => meal.category == "China");
  chinese.forEach(items => {
    sectionDOM.innerHTML += (" " + foodAdd(items))
  });
}
// window: load event tüm menüleri başlangıçta sayfaya çağırma
